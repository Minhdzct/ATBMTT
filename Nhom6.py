...
class DSA_GUI:
    ...
    def setup_style(self):
        ...
        s.configure("Treeview", background="#fdfdfd",
                    fieldbackground="#fdfdfd", rowheight=28)
    ...
    def _on_wheel(self, ev):
        direction = -1 if (ev.num == 4 or ev.delta > 0) else 1
    ...
    def build_section(self, parent, title, builder, side):
        ...
        frame.pack(side=side, fill=tk.BOTH, expand=True,
                   padx=(0, 12) if side == tk.LEFT else (12, 0))
        builder(frame)
    ...
    def build_params(self, parent):
        ...
        cb = ttk.Combobox(top, textvariable=self.ln_var,
                          values=opts, width=18, state="readonly")
        cb.pack(side=tk.LEFT, padx=6)

        self.btn_params = ttk.Button(
            top, text="Tạo Tham số", style="Accent.TButton", command=self.generate_params
        )
        self.btn_params.pack(side=tk.LEFT)

        # ⭐ NÚT NHẬP THỦ CÔNG
        self.btn_manual = ttk.Button(
            top, text="Nhập thủ công", style="Accent.TButton",
            command=self.manual_params_window
        )
    ...
    # -------------------------
    # CỬA SỔ NHẬP THỦ CÔNG
    # -------------------------
    def manual_params_window(self):
        win = tk.Toplevel(self.root)
        win.title("Nhập tham số thủ công")
        ...
        for i, name in enumerate(labels):
            ttk.Label(win, text=f"{name} =").grid(
                row=i, column=0, padx=10, pady=5, sticky="w")
            entry = ttk.Entry(win, width=75)
            entry.grid(row=i, column=1, padx=10, pady=5)
            self.manual_entries[name] = entry

        # ⭐ CẢNH BÁO
        warning_text = (
            "⚠ Lưu ý: tham số nhỏ rất dễ làm verify sai!\n\n"
            "❗ Lý do:\n"
            "- Không gian quá nhỏ → H nhiên (SHA-256) lớn hơn nhiều so với q.\n"
            "- k, r, s có thể trùng lặp → chữ ký không ổn định.\n"
            "- Một số p–q nhỏ thỏa lý thuyết nhưng không phù hợp cho DSA\n"
            "  (DSA thiết kế cho p ≥ 1024 bit)."
        )

        label_warn = ttk.Label(
            win, text=warning_text, foreground="red", justify="left", anchor="w"
        )
        label_warn.grid(row=3, column=0, columnspan=2,
                        padx=10, pady=(5, 10), sticky="w")

        ttk.Button(
            win, text="Xác nhận", style="Accent.TButton",
            command=lambda: self.set_manual_params(win)
        ).grid(row=5, column=0, columnspan=2, pady=10)

    # -------------------------
    # XỬ LÝ THAM SỐ THỦ CÔNG
    # -------------------------
    def set_manual_params(self, window):
        try:
            p = int(self.manual_entries["p"].get().strip())
            q = int(self.manual_entries["q"].get().strip())
            g = int(self.manual_entries["g"].get().strip())

            if (p - 1) % q != 0:
                raise ValueError("q không phải ước của p - 1")

            if pow(g, q, p) != 1:
                raise ValueError("g không hợp lệ (g^q mod p phải = 1)")

            self.p, self.q, self.g = p, q, g

            self._update(self.p_txt, p)
            self._update(self.q_txt, q)
            self._update(self.g_txt, g)

            self.btn_keys.config(state="normal")
            self.update_summary_table()

            window.destroy()
            messagebox.showinfo("OK", "Đã nhập tham số thành công!")
    ...
    def build_keys(self, parent):
        self.btn_keys = ttk.Button(
            parent, text="Tạo Cặp Khóa", style="Accent.TButton",
    ...
    def build_sign(self, parent):
        ...
        self.btn_choose = ttk.Button(
            btn_row, text="Chọn Tệp...", command=self.choose_file)
        self.btn_choose.pack(side=tk.LEFT)

        self.file_label = ttk.Label(
            btn_row, text="Chưa chọn tệp", anchor="w")
        self.file_label.pack(side=tk.LEFT, padx=8)

        ttk.Label(parent, text="Văn bản (M) hoặc đường dẫn file:").pack(
            anchor="w", pady=(6, 4))

        self.msg_sign = scrolledtext.ScrolledText(
            parent, height=3, font=("Consolas", 10))
        self.msg_sign.pack(fill=tk.X, pady=(0, 8))
        self.msg_sign.insert(tk.END, "Đây là một thông điệp kiểm tra.")

        self.btn_sign = ttk.Button(
            parent, text="Ký (file hoặc nội dung)", style="Accent.TButton",
    ...
    def build_verify(self, parent):
        ...
        self.btn_choose_verify = ttk.Button(
            btn_row, text="Chọn Tệp Xác Minh...", command=self.choose_verify_file)
        self.btn_choose_verify.pack(side=tk.LEFT)

        self.verify_file_label = ttk.Label(
            btn_row, text="Chưa chọn tệp xác minh", anchor="w")
        self.verify_file_label.pack(side=tk.LEFT, padx=8)

        ttk.Label(parent, text="Văn bản / File để xác minh (M')").pack(
            anchor="w", pady=(6, 4))

        self.msg_verify = scrolledtext.ScrolledText(
            parent, height=3, font=("Consolas", 10))
        ...
        ttk.Label(grid, text="r':", width=6).grid(
            row=0, column=0, sticky="w", padx=(0, 5))

        self.r_entry = ttk.Entry(grid, font=("Consolas", 10))
        self.r_entry.grid(row=0, column=1, sticky="ew")
        grid.columnconfigure(1, weight=1)

        ttk.Label(grid, text="s':", width=6).grid(
        ...
        self.btn_verify = ttk.Button(
            parent, text="Xác Minh (dùng file đã chọn nếu có)",
            style="Accent.TButton", command=self.verify_signature,
            state="disabled"
        )
    ...
    def build_summary_table(self, parent):
        frame = ttk.Labelframe(parent, text="7. Các Giá Trị",
                               padding="12 10")
    ...
    def labeled_field(self, parent, label, row):
        ...
        ttk.Label(f, text=label, width=26,
                  anchor="w", font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)

        txt = scrolledtext.ScrolledText(
            f, height=2, font=("Consolas", 9),
    ...
    def update_summary_table(self):
        values = {
            ...
            "v": self.last_v,
            "Kết quả xác minh":
                "HỢP LỆ" if self.result_label["text"] == "HỢP LỆ"
                else "KHÔNG HỢP LỆ" if self.result_label["text"] == "KHÔNG HỢP LỆ"
                else "—"
        }
        ...
        for i, (var, _, _) in enumerate(self.tree_data):
            if var in values:
                self.tree.item(
                    ...
                    values=(var, self.tree_data[i][1], values[var])
                )
    ...
    def choose_file(self):
        path = filedialog.askopenfilename(
            title="Chọn tệp", filetypes=[("All files", "*.*")]
        )
        if not path:
            return
    ...
    def choose_verify_file(self):
        path = filedialog.askopenfilename(
            title="Chọn tệp xác minh", filetypes=[("All files", "*.*")]
        )
        if not path:
            return
    ...
    def generate_keys(self):
        if not self.p:
            return messagebox.showwarning("Cảnh báo", "Tạo tham số trước.")
    ...
    def sign_message(self):
        if not self.x:
            return messagebox.showwarning("Cảnh báo", "Tạo khóa trước.")
        ...
        self.msg_verify.delete("1.0", tk.END)
        self.msg_verify.insert(tk.END, f"{source}")
    ...
    def verify_signature(self):
        if not self.y:
            return messagebox.showwarning("Cảnh báo", "Cần khóa + chữ ký.")
        ...
        is_valid, w, u1, u2, v, log_lines = self.dsa.verify_bytes(
            self.p, self.q, self.g, self.y, data, r, s
        )
        ...
        self.last_H = int(hashlib.sha256(data).hexdigest(), 16)
        self.last_w, self.last_u1, self.last_u2, self.last_v = w, u1, u2, v
        ...
        if is_valid:
            self.result_label.config(
                text="HỢP LỆ", foreground="#10b981", font=("Segoe UI", 16, "bold"))
        else:
            self.result_label.config(
                text="KHÔNG HỢP LỆ", foreground="#ef4444", font=("Segoe UI", 16, "bold"))
        ...
        if is_valid:
            messagebox.showinfo(title, msg_popup)
        else:
            messagebox.showwarning(title, msg_popup)
